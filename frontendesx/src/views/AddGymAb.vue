            ...
            <v-list-item v-for="gym in gyms" :key="gym.id">
              ...
              <v-list-item-action>
                <v-btn icon @click="dialog = true">
                  <v-icon color="grey lighten-1">mdi-view-grid-plus</v-icon>
                </v-btn>
                <v-dialog v-model="dialog" max-width="290">
                  <v-card>
                    ...
                    <v-card-text>
                      Sigur vrei sa adaugi Sala in Abonament?
                    </v-card-text>
                    ...
                    <v-card-actions>
                      ...
                      <v-btn color="green darken-1" text @click="add(gym.id)">
                        Agree
                      </v-btn>
                    </v-card-actions>
                  </v-card>
...
export default {
  ...
  methods: {
    async init() {
      try {
        ...
        var result = await axios.get(
          "http://localhost:8081/abonament/getSalaNotAb/" + this.idAb
        );
    ...
    async add(id) {
      try {
        await axios.post("http://localhost:8081/abonament/addArc", {
          idAbonament: this.idAb, ///this.$route.params.idAb,
          idSala: id,
        });
        this.dialog = false;
        // created();
